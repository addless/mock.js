...
(function () {
    ...
    function parse () {
        ...
        return parse.apply(parser, arguments);
    }
    ...
    function testsExecutor(executeFn) {
        ...
        for(var i = 0; i < data.length; i++) {
            context = this.deriveContext(data[i]);
            describe('', function () {
                it('should ' + context.description, getItFunction.call(me, context));
            });
        }
        return this;

        function getItFunction(context) {
            var driver = this;
            return function (done) {
                executeContext.apply(driver, [context, done]);
            }
        }
    }
        ...
        try {
            for (var i = 0; i < ctx.children.length; i++) {
                ...
                function execute(scope) {
                    executeFn.apply(scope || testScope, [done]);
                }
            }
        // } catch (e) {
